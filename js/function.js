;(function() {
	...
	*/
	...
	class Placement {
		// объект с координатами сторон игрового поля
	...
	class Controller {
		...
		init() {
			...
			if (this.player === human) {
				compShot = false;
				this.text = 'Ви робите вистріл першим.';
			} else {
				compShot = true;
				this.text = 'Першим робить вистріл ворог.';
				// выстрел компьютера
		...
		makeShot(e) {
			...
			switch(v) {
				...
				case 4:
					Controller.showServiceText('Ви вже стріляли за цими координатами!');
					break;
			}
		}
		...
		miss(x, y) {
			...
			if (this.player === human) {
				text = 'Ви не влучили. Вистріл робить ворог.';
				this.player = computer;
				this.opponent = human;
				compShot = true;
				setTimeout(() => this.makeShot(), 2000);
			} else {
				text = 'Ворог промахнувся.Ваш вистріл.';
		...
		hit(x, y) {
			...
			text = (this.player === human) ? 'Вітаємо, ви влучили. Ваш вистріл.' : 'Ворог влучив у Ваш корабель. Вистріл ворога.';
			setTimeout(() => Controller.showServiceText(text), 400);
